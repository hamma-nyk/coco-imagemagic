...
export default function ToolClientContent({ slug }: { slug: string }) {
  const renderTool = () => {
    switch (slug) {
      ...
      case 'colorize': return <ColorizeTool />; // Route baru
      case 'enhance': return <EnhanceTool />;   // Route baru
      default: return (
        <div className="text-center py-20 border border-dashed border-white/10 rounded-3xl">
          <p className="text-slate-500 font-mono">MODULE_NOT_FOUND: ERROR_404</p>
          <Link href="/" className="text-cyan-400 text-sm underline mt-4 inline-block italic">
  ...
  return (
    <div className="max-w-4xl mx-auto px-6 py-12">
      <div className="mb-10">
        <Link href="/" className="text-xs font-mono text-slate-500 hover:text-cyan-400 transition-colors uppercase tracking-widest">
